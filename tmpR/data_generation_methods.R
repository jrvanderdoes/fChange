...
fgarch11 <- function(N, J, delta=0.01, burn_in=50) {
  ...
  for (i in 2:(N+burn_in)) {
    for (u in 1:J) {
      ...
      sigma2_mat[u,i] <- delta + dot_integrate(alpha_op_vec) + dot_integrate(beta_op_vec)
    }
    garch_mat[,i] <- sqrt(sigma2_mat[,i]) * error_mat[,i]
  }

  funts(garch_mat[,burn_in + 1:N])
}
...
#'
#' @examples
generateFMA1 <- function(resolution, N, burnin=1000){
