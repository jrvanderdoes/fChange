...
#' @param ... Additional parameters to pass into the `trim_function()` or
#' `compute_Mn()`.
#'
...
.autoElbow_method <- function(data,
  ...
  return_data[1,] <- c(tmp_loc, .compute_total_var(data, tmp_loc, errorType), 1)
...
.demean <- function(data, CPs){
  ...
  for(i in 1:(length(CPs)-1)){
    data_demean[,(CPs[i]+1):CPs[i+1]] <-
      data_demean[,(CPs[i]+1):CPs[i+1]] -
      rowMeans(data_demean[,(CPs[i]+1):CPs[i+1]])
  }
