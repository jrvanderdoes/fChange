...
#'
#' @noRd
.estimCovMat1 <- function(X, x = seq(0, 1, length.out = nrow(X)),
...
#'
#' @noRd
.estimD1 <- function(K, h, X, lfun, v, lfunp, vp) {
  ...
  values <- apply(data_tmp, MARGIN = 1, function(kInfo, X1, fVals, fpVals, mean1, mean2) {
    kInfo[1] * .estimGamma(
      k = kInfo[2], X = X1,
