...
#' @param alpha Numeric in \[0, 1\] for the significance level of alpha
#' @param silent Boolean for if anything should be output
#' @param ... Additional parameters
#'
#' @return List with the following items:
#'  \itemize{
#'    \item 'tn': Tn test statistic
#'    \item 'mn': Mn test statistic
#'    \item 'location': Estimate for the change location
#'    \item 'cutoff_tn': Cutoff for the Tn test statistic
#'    \item 'cutoff_mn': Cutoff for the Mn test statistic
#'    \item 'pval_tn': p-value for Tn
#'    \item 'pval_mn': p-value for Mn
#'    \item 'data_bs': Bootstrap samples of the data
#'  }
#' @export
#'
#' @examples
...
TnMn_resampling <- function(X, blocksize = 1, iters = 1000,
  ...
  bssamples <- .bootstrap(X,blocksize,iters,type,replace,fn=compute_TnMn, ...)
  # bssamples <- unlist(lapply(boot_dat,compute_TnMn, ...))
