...
#' @param look_ahead Integer for the number of nodes to look ahead at and
#'  confirm a flattening. Default is 2.
#' @param look_alpha Numeric to quantify the gain required of each additional
...
elbow_method <- function(X,
  ...
  # Run First
  stats_tmp <- test_function(X$data[, (1 + trim_amt):(n - trim_amt)], ...)
  ...
  return_data[1, ] <- c(tmp_loc,
                        .compute_total_var(data = X$data, CPs = tmp_loc,
                                           errorType = errorType,K=K))
  ...
  while (nrow(return_data) < max_changes) {
    ...
    if (1 + trim_amt < length(beforePrevCP) - trim_amt) {
      ...
      stats_tmp <- test_function(X$data[, beforePrevCP[fill_idx]],
                                 J = length(fill_idx), ...)
    ...
    if (1 + trim_amt < length(afterPrevCP) - trim_amt) {
      ...
      stats_tmp <- test_function(X$data[, afterPrevCP[fill_idx]],
                                 J = length(fill_idx), ...)
      # stats_tmp$allValues[1] <- 0 # Make first zero as don't want to segment
    ...
    for (k in 1:length(data_segments)) {
      ...
      return_data_tmp[k, ] <- c(
        section_max,
        .compute_total_var(X$data, tmp, errorType,K=K)
      )
    }
  ...
  return_data <- rbind(
    c(NA, .compute_total_var(X$data, c(), errorType,K=K)),
    return_data
  )
...
#'
#' @noRd
.compute_total_var <- function(data, CPs, errorType = "L2", M = 1000, K=bartlett_kernel) {
  # Setup
  ...
  if (errorType == "L2" || errorType == "Tr") {
    ...
    covMatrix <- .long_run_cov(data_std, h=0, K=K)


  } else if (errorType == "CE") {
    # W <- as.data.frame(sapply(rep(0, M), sde::BM, N = nrow(data) - 1))
    W <- generate_brownian_motion(M, v=seq(0,1,length.out=nrow(data)) )$data
