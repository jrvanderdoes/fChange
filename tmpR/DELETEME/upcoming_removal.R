...
generate_far1 <- function(resolution, N, d=1/2, burnin=1000){
  ...
  ## TODO:: Remove pracma
  c <- d * 1 / sqrt(pracma::quad2d(function(x,y,K){ K(x,y)^2 },
                                   xa = 0,xb = 1,ya = 0, yb = 1,K=K) )
  # d * (1 / sqrt(cubature::adaptIntegrate(
  #   function(x,K){ K(x[1],x[2])^2 },
  #   c(0, 0), c(1, 1), K=K)$integral))
...
.generateXi <- function(dist, sd, n = 1, dof=NULL) {
  ...
  } else if (tolower(dist) == "exponential") {
    xi <- stats::rexp(n, rate = 1 / sd) - sd
  } else if (tolower(dist) == "t") {
    xi <- stats::rt(n, dof)
    if(dof>2)
      xi <- xi * sqrt(sd^2 * (dof - 2) / dof)
  } else if (tolower(dist) == "cauchy") {
    xi <- stats::rt(n, 1)
  } else if (tolower(dist) == "laplace") {
