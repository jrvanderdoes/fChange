...
scalarDetection <- function(Y, gam = 0.5, nSims = 200,
  ...
  for (k in 1:(n - 1)) {
    Tn_s[k] <- ((k * (n - k)) / n^2)^gam * ((k * (n - k)) / n) *
      ...
      # dot_integrate(abs(.phi_k(Y, ts, k) - .phi_k0(Y, ts, k))^2 * .w(ts))
  }
  Tn <- max(Tn_s)

  Tn_permute <- sapply(rep(NA, nSims),
    function(tmp, n, gam, Y, ts) {

      Tn_tmp_s <- sapply(1:(n - 1), function(k, n, gam, Y, ts) {
        YY <- Y[sample(1:length(Y))]
        ...
        # dot_integrate(abs(.phi_k(YY, ts, k) - .phi_k0(YY, ts, k))^2 * .w(ts))
      }, n = n, gam = gam, Y = Y, ts=ts)
