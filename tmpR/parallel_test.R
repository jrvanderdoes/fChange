...
#' Detect Change Point - Parallel
#'
#' This function detects change points in functional data. This is done through
#'     simulating the null distribution and completing in parallel
#'
...
#'     barlett_kernel.
#' @param cores (Optional) Integer indicating the number of cores to use. If NA,
#'     uses one less than the total available cores. Default is NA.
...
#'            evals = seq(0,1,0.05),
#'            kappasArray = c(0))
#' tmp <- detect_changepoint_parallel(X)
#' tmp$pval
#'
...
detect_changepoint_parallel <- function(X, nSims=100,
  ...
  # Set up cores
  if(is.na(cores)) cores <- detectCores()-1 # Don't want to use whole comp
  cl <- makeCluster(cores)
  registerDoParallel(cl)

  gamProcess <- foreach(i=1:nSims, .combine=c#, .packages='fChange'
                        ) %dopar% {
                          {
...
for(i in 1:500){
  ...
  tictoc::tic()
  detect_changepoint_parallel(X, cores=7)
  tictoc::toc()
}
