...
for(i in 1:nTrials){
  cat(paste0(i,', '))
  X <- .generateFunctionalIID(21,500)

  tmp <- detect_changepoint_final_Mn(X,M=30, nSims = 1000,h=0)
  pvals[i] <- tmp$pval
  pvals1[i] <- tmp$pval1
  pvals2[i] <- tmp$pval2
}
...
#'
#' @noRd
.estimCovMat_final <- function(X, W, x=seq(0,1,length.out=nrow(X)),
