opt2 <- function(X,x,Cov_M,h,K,W){
  ...
  .estimD <- function(K,h,X,lfun,v,lfunp,vp){
    ...
    KVals <- sapply(iters, K, h=h) # Can vectorize eventually
    # K1 <- function(l, h=10) ifelse(h==l&h==0,1,ifelse(abs(l)<h,1-abs(l) / h,0))

    # Move so pass in function values to avoid recomputation later
    mean1 <- mean(.estimf(X,lfun,v))
    mean2 <- mean(.estimf(X,lfunp,vp))

    values <- sapply(iters, function(k,K,h,X1,lfun,v,lfunp,vp,mean1,mean2){
           Kval <- K(k,h)

           ifelse(Kval==0,
                  0,
                  Kval * .estimGamma(k=k,X=X1,lfun=lfun,v=v,lfunp=lfunp,vp=vp,
                                     mean1=mean1,mean2=mean2))
    },K=K,h=h,X1=X,lfun=lfun,v=v,lfunp=lfunp,vp=vp,mean1=mean1,mean2=mean2)
  ...
  .estimGamma <- function(k,X,lfun,v,lfunp,vp,mean1,mean2){
    tmp <- 0
    ...
    values <- sapply( rs,
                      function(r,X1,lfun,v,lfunp,vp,mean1,mean2){
                        .estimR(X1,r,lfun,v,mean1) * .estimR(X1,r+k,lfunp,vp,mean2)
                      },X1=X,lfun=lfun,v=v,lfunp=lfunp,vp=vp,
                      mean1=mean1,mean2=mean2)
  ...
  round(.estimCovMat(X,x,Cov_M,h,K,W),13)
}
