...
Although the package is available on [CRAN](https://cran.r-project.org/web/packages/fChange/index.html), several functions have been
updated in preparation for this document and have not yet been uploaded to CRAN. Hence, we recommend installing the code from GitHub as:
```{r,eval=FALSE}
...
The slides can be reached [here](SSC_Short_Course.png).
...
length $T=504$ is shown in Figure [1.1](#Fig-spot3){reference-type="ref"
reference="Fig-spot3"}. This depicts electricity spot prices in Spain
...
future values. The time series shown in Figure
[1.1](#Fig-spot3){reference-type="ref" reference="Fig-spot3"} exhibits
...
2014.***](functionaldata/tweeks.png){#Fig-spot3 width="5in"}
...
the data to Figure [1.2](#Fig-spot4){reference-type="ref"
reference="Fig-spot4"}. Such visualizations are shown in Figures
[1.3](#Fig-spot5){reference-type="ref" reference="Fig-spot5"} and
[1.4](#Fig-spot6){reference-type="ref" reference="Fig-spot6"}. There we
...
visualized as a functional time series.***](functionaldata/e1.png){#Fig-spot5 width="5in"}

![ ***Another angle of hourly electricity spot price in Spain during the
year 2014 visualized as a functional time
series.***](functionaldata/electricity_data.png){#Fig-spot6 width="5in"}
...
Equation [\[cons\]](#cons){reference-type="eqref" reference="cons"}, in
...
indexed by a continuous domain. For example, the spot price data
visualized in Figure [1.3](#Fig-spot5){reference-type="ref"
reference="Fig-spot5"} we can view as being indexed on the continuous
...
take advantage of [\[cons\]](#cons){reference-type="eqref"
reference="cons"} and construct for each $t\in [0,1]$ $$\begin{aligned}
\label{smoother}
...
B-spline polynomials based on equally spaced knots is shown in Figure
[1.9](#Fig-spl){reference-type="ref" reference="Fig-spl"}.

![ A plot of cubic B-spline polynomials based on equally spaced
knots.](functionaldata/spline.png){#Fig-spl width="4.55in"}
...
splines.](functionaldata/splex.png){#Fig-spl width="3.75in"}

After choosing a basis, we still must select $K$ and the coefficient
vector ${\bf c}$ in [\[smoother\]](#smoother){reference-type="eqref"
reference="smoother"}. This is typically done using least squares: we
...
Suppose that the Spanish electricity data in Figure
[1.4](#Fig-spot6){reference-type="ref" reference="Fig-spot6"} was
...
![ ***Fast rainbow spaghetti plot of the FTS French mortality data.***](functionaldata/fmortality_fast.png){#Fig-elam2 width="5in"}

![ ***Rainbow plot of the FTS French mortality data.***](functionaldata/fmortality_rainbow.png){#Fig-elam1 width="5in"}
...
to decompose the FTS as in [\[cons\]](#cons){reference-type="eqref"
reference="cons"}. This turns out to be a somewhat special expansion for
a stationary FTS called the *Karhunen**--**Loéve (KL) expansion*. It
takes the form $$\label{e:KL}
...
choices of the functions $v_1,...,v_d$ in
[\[fin-dim\]](#fin-dim){reference-type="eqref" reference="fin-dim"}.
...
data.](functionaldata/fmortality_skree.png){#fig:fmortality_skree width="40%"}
...
![ ***The first PC scores.***](functionaldata/fmortality_coef1.png){width="5in"}
...
[\[forecast\]](#forecast){reference-type="eqref" reference="forecast"}
...
how much they might deviate from the forecasted curve in
[\[forecast\]](#forecast){reference-type="eqref" reference="forecast"}.
...
error\" in [\[hu\]](#hu){reference-type="eqref" reference="hu"} is
...
In the case of model [\[swn\]](#swn){reference-type="eqref"
reference="swn"}, the best predictor of $x_n$ given past values of the
series is the mean value $\mu$, which can be estimated from
$x_1,...,x_{n-1}$ by the sample mean $\bar{x}$. Model
[\[rw\]](#rw){reference-type="eqref" reference="rw"} on the other hand
can be rewritten as
$$x_t = \mu_1 + x_{t-1} + w_t  \; \iff \; \nabla x_{t} =  x_t - x_{t-1} = (1-B)x_t=  \mu_1 + w_t,$$
which makes clear that the best forecast for $x_n$ based on
$x_{n-1},...,x_1$ in this case is $x_{n-1} + \mu_1$. The model
[\[rw\]](#rw){reference-type="eqref" reference="rw"} is also called an
...
recursion for $x_t$ in [\[SARIMA\]](#SARIMA){reference-type="eqref"
reference="SARIMA"}, we can simulate approximate future values of the
series $x_{T+h}^{(b)}$.
...
$h$*, $h \ge 0$, is defined by $$\label{e:cov_op}
...
implemented in the function `portmanteau_tests()`.
...
![***Residuals from the arima-based Hyndman-Ullah model.***](functionaldata/electricity_residuals.png){#fig:electricity_residuals_mlwn
width="5in"}

![***Autocorrelation of the residuals from the arima-based Hyndman-Ullah model.***](functionaldata/electricity_residuals_sacf.png){#fig:electricity_residuals_mlwn
width="5in"}

![***Multi-lag white noise portmanteau tests' p-values for the residuals
from the arima-based Hyndman-Ullah
model.***](functionaldata/electricity_residuals_mlwn.png){#fig:electricity_residuals_mlwn
width="5in"}
...
One stationary test is implemented as `stationary_test()`. The Spanish
electricity data has a p-value $<0.001$ for the stationary test. The
first difference and the model residuals both return non-significant
p-value, suggesting stationarity.

Applying change point detection on the electricity data, two change
point are observed; see Figure
[3.5](#fig:electricity_changes){reference-type="ref"
reference="fig:electricity_changes"}. Since this suggests regions of
homogeneity in the data, CV was re-run to compare the previous models to
models based on only the last homogeneous segment: 2281 (ets), 2522
(arima), 2426 (ets with cp), and 2784 (arima with cp). Although the data
in the last segement appears to be more homogenous, in this case the
reduction of data lead to worse CV performance.
