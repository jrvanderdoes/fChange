...
```{r, eval=FALSE}
plot(electricity, plot_title = NULL,
...
```{r, eval=FALSE}
urlfile1 <-  paste0('https://raw.githubusercontent.com/jrvanderdoes/fChange',
              '/main/vignettes/articles/pollution.txt')
city <- read.csv(urlfile1, header = TRUE,sep=',')$x
...
```{r, eval=FALSE}
urlfile2 <-  paste0('https://raw.githubusercontent.com/jrvanderdoes/fChange',
              '/main/vignettes/articles/log_fmort.csv')
fmort <- read.csv()
fmort <- dfts(fmort,labels=1816:2022,fparam=0:100)
...
```{r, eval=FALSE}
uneven_spanish <- readRDS('./vignettes/articles/uneven_spanish.rds')
...
for(i in 1:length(uneven_spanish)){
  long_data <-
    rbind(long_data, 
          data.frame('x'= i + 0:(length(uneven_spanish[[i]]$values)-1) / 
...
plot(spanish_electricity,plot_title = NULL,
...
```{r eval=FALSE}
pca_data <- pca(log_fmort_fdts, TVE=0.95)
...
```{r eval=FALSE}
elec_ets <- projection_model(X = electricity, forecast.model = 'ets', TVE = 0.99, 
                        n.ahead = 30, check.cp = F, alpha = 0.05,
                        sim.bounds = TRUE, M = 1000)
elec_arima <- projection_model(X = electricity, forecast.model = 'arima', TVE = 0.99, 
                        n.ahead = 30, check.cp = F, alpha = 0.05,
                        sim.bounds = TRUE, M = 1000)
...
for(i in 1:length(pred_idx)){
  ...
  pm1 <- projection_model(tmp_dat,forecast.model='ets',n.ahead=1,
                          check.cp = F,sim.bounds = F)
  pm2 <- projection_model(tmp_dat,forecast.model='arima',n.ahead=1,
                          check.cp = F,sim.bounds = F)
...
```{r eval=FALSE}
fit <- projection_model(electricity,check.cp = FALSE);
fit_log <- projection_model(electricity,transformation='log',check.cp = FALSE);
fit_sqrt <- projection_model(electricity,transformation='sqrt',check.cp = FALSE)
...
```{r eval=FALSE}
elec_log_ets <- projection_model(X = electricity, forecast.model = 'ets', TVE = 0.99, 
                        n.ahead = 30, check.cp = F, alpha = 0.05,
                        sim.bounds = TRUE, M = 1000,transformation = 'log')
elec_log_arima <- projection_model(X = electricity, forecast.model = 'arima', TVE = 0.99, 
                        n.ahead = 30, check.cp = F, alpha = 0.05,
                        sim.bounds = TRUE, M = 1000,transformation = 'log')
...
colnames(errors) <- c('fets','farima','fetsLog','farimaLog')
...
for(i in 1:length(pred_idx)){
  ...
  pm2 <- projection_model(tmp_dat,forecast.model='arima',n.ahead=1,
                          check.cp = F,sim.bounds = F)
  pm3 <- projection_model(tmp_dat,forecast.model='ets',n.ahead=1,
                          check.cp = F,sim.bounds = F,transformation = 'log')
  pm4 <- projection_model(tmp_dat,forecast.model='arima',n.ahead=1,
                          check.cp = F,sim.bounds = F,transformation = 'log')
...
Consider the pollution data. This time we will compare a functional approach to traditional methods.
```{r eval=FALSE}
...
for(i in 1:length(pred_idx)){
  ...
  arima_for  <- forecast::forecast(tmp_scalar, h = h*48, level = 95)
  scalar_ets <- forecast::ets(tmp_scalar)
  ets_for  <- forecast::forecast(tmp_scalar, h = h*48, level = 95)
  
  pm1 <- projection_model(tmp_dat,forecast.model='ets',n.ahead=h,
                          check.cp = F,sim.bounds = F)
  pm2 <- projection_model(tmp_dat,forecast.model='arima',n.ahead=h,
                          check.cp = F,sim.bounds = F)
  pm3 <- projection_model(tmp_dat,forecast.model='ets',n.ahead=h,
                          check.cp = F,sim.bounds = F,transformation = 'log')
  pm4 <- projection_model(tmp_dat,forecast.model='arima',n.ahead=h,
                          check.cp = F,sim.bounds = F,transformation = 'log')
...
pm1 <- projection_model(city_dfts,forecast.model='ets',n.ahead=h,
                        check.cp = F,sim.bounds = F)
pm2 <- projection_model(city_dfts,forecast.model='arima',n.ahead=h,
                        check.cp = F,sim.bounds = F)
pm3 <- projection_model(city_dfts,forecast.model='ets',n.ahead=h,
                        check.cp = F,sim.bounds = F,transformation = 'log')
pm4 <- projection_model(city_dfts,forecast.model='arima',n.ahead=h,
                        check.cp = F,sim.bounds = F,transformation = 'log')
...
true <-read.csv(
  paste0('C:/Users/jerem/OneDrive/Documents/School/Waterloo/Research/RPackages/fChange/vignettes/articles/pollution_true.txt'))[,2]
res <- colSums((ests_final-true)^2)
res / min(res)
```

We see that CV recommend farima, but all functional models far outperform the scalar. Scaled the CV results were: 
2.45 (arima), 2.453 (ets), 1.450 (fets), 1.000 (farima), 1.271 (log fets), and 1.031 (log arima). When forecasting and comparing to the true future, similar results are seen: 1.601 (arima), 1.601 (ets), 1.196 (fets), 1.000 (farima), 1.866 (log fets), and 1.073 (log arima).

![ ***The pollution forecast model.***](functionaldata/pollution_for.png){ width="5in"}
```{r, eval=FALSE}
mod <- projection_model(city_dfts,forecast.model='arima',n.ahead=h,
                        check.cp = F,sim.bounds = F)
...
plot(electricity, changes=elec_changes$location,
     plot_title = NULL,
     eye = list(x = -1, y = -1.5, z = 0.75),
     aspectratio=list(x = 0.8, y = 0.6, z = 0.6)
)
plot(electricity, changes=elec_changes$location, type='change',
     plot_title = NULL,
     eye = list(x = -1, y = -1.5, z = 0.75),
     aspectratio=list(x = 0.8, y = 0.6, z = 0.6)
)
...
for(i in 1:length(pred_idx)){
  ...
  pm2 <- projection_model(tmp_dat,forecast.model='arima',n.ahead=1,
                          check.cp = F,sim.bounds = F)
  pm3 <- projection_model(tmp_dat1,forecast.model='ets',n.ahead=1,
                          check.cp = F,sim.bounds = F)
  pm4 <- projection_model(tmp_dat1,forecast.model='arima',n.ahead=1,
                          check.cp = F,sim.bounds = F)
  ...
  ests <- data.frame(
    ...
    'fetsC'=as.numeric(
      pm3$data$component_true$data[,idx-cp+1]),
    'farimaC'=as.numeric(
      pm4$data$component_true$data[,idx-cp+1])
  )
