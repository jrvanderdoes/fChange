...
epca <- pca_examination(X = funts(electricity),TVE = 0.99) 
set.seed(654321)
...
set.seed(123456)
elec_mod <- model_pca(electricity,TVE=0.95,model='ets', check.cp=TRUE)

plot(elec_mod$errors,CPs = elec_mod$CPs) 
```
...
set.seed(25641)
spy_min_cps <- change(funts(SPYUS_ocidr), type='segmentation',
                                   statistic='Tn',
                                   critical='simulation',
