test_that("Center works", {
  ...
  expect_equal(matrix(center(funts(data.frame(rep(1,3),rep(2,3),rep(3,3))))$data,3,3),
               matrix(rep(-1:1,time2=3),3,3,byrow = TRUE))
})

test_that("PCA works", {
  expect_equal(round(pca(1:10)$sdev,5), 3.02765)
  expect_equal(round(pca(funts(electricity))$sdev[1:3],6),
               c(15.764828, 5.456613, 3.313120))
})

test_that("SD works", {
  expect_equal(round(sd(1:10),5), 3.02765)
  expect_equal(round(sd(funts(electricity))[1:3],5),
               c(16.35713, 16.34201, 16.39898))
})

test_that("Var works", {
  expect_equal(round(var(1:10),5), 9.16667)
  expect_equal(round(var(funts(electricity),type='pointwise')[1:3],5),
               c(267.55565, 267.06142, 268.92667))
  expect_equal(round(diag(var(funts(electricity),type='operator'))[1:3],5),
               c(266.82262, 266.32974, 268.18988))
})

test_that("CUSUM works", {
  expect_equal(round(cumsum(funts(electricity))$data[c(1,5,10),365],5),
