...
permutate_Tn <- function(X, blockSize, iters = 1000,
  ...
  .compute_Loc <- function(CF){
    ...
    list('loc'=which.max(1/nrow(fVals) *
                     rowSums(fVals)),
         'locPlot'=ggplot() + theme_bw() +
           geom_line(aes(x=nx[-1],y=1/nrow(fVals) * rowSums(fVals)))
         )
  }
  ...
  bssamples <- sapply(as.data.frame(idxs),
                      function(loop_iter,X1){
                        .compute_Tn_CF(X1[na.omit(loop_iter),]) },
                      X1=CF)

  append(list('cutoff'=quantile(bssamples, probs = c(1-alpha))[[1]],
    'stat'=fullVal), .compute_Loc(CF))
}
