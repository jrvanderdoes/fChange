...
#   sum1 <- sum(
#     sapply(1:K, function(i,f_data,M,low_disc){
...
#   mean_V_K <- K * (sum(diag(cov)) / J)^2
#
#   # Var
#   # var_V_K <- variance_hat_V_K_iid(f_data, K)
#   var_V_K <- K * 2 * ( sum(cov^2) / J^2 )^2
#
#   # Welch Param
#   beta <- var_V_K / (2 * mean_V_K)
#   nu <- 2 * mean_V_K^2 / var_V_K
...
#     }
#     bootsraps <- list()
#     # bootstrap_samples <- block_bootstrap(f_data, block_size, B = straps, moving = moving)
#     bootstrap_samples <- .bootstrap(f_data, blocksize = block_size,
#                                     M = straps, type =  ifelse(moving,'overlapping', 'separate'),
#                                     replace = TRUE)
...
#   cov_iid <- obtain_autocovariance(f_data,0)
#
